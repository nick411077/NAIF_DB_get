...
myclient = pymongo.MongoClient(
    "mongodb+srv://nick411077:nick10279@nickcandiscordbot-iqnfx.gcp.mongodb.net/test?retryWrites=true&w=majority")  # 位置
...
def mongoOutG(DATA):
    ...
    for bd in BD:
        for i in range(len(DATA)):
            if bd.get("日期") == DATA[i].get("日期") and bd.get("資料來源") == DATA[i].get("資料來源") and bd.get("類別") == DATA[
                i].get("類別") and bd.get("地區") == DATA[i].get("地區"):
                YN[i] = False
    for i in range(len(DATA)):
        if YN[i]:
